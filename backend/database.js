...
// Free tier: /tmp is always writable on Render but wipes on redeploy.
// Bookings and customers reset on redeploy — unavoidable without a paid disk.
// Cars use INSERT OR IGNORE so they are re-seeded on every startup,
// meaning the fleet is always present even after a wipe.
...
function initDB() {
  ...
  // INSERT OR IGNORE means this runs on every startup safely.
  // If the row already exists it is skipped; if /tmp was wiped it is recreated.
  // is_available is NOT forced back to 1 here — a separate UPDATE below
  // only resets availability for cars that have no active booking,
  // so a mid-session rental isn't accidentally unlocked on restart.
  ...
  const seed = [
    ['C001','Toyota','Camry',2023,'sedan',60,5,'Petrol','Automatic','2.5L 4-Cyl','203 hp','250 Nm','210 km/h','8.1s (0-100)','15 km/L','428 L',
      ...
      'linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%)','#4c8ac9','#b0c4e0',1],
    ['C002','Honda','Accord',2023,'sedan',70,5,'Petrol','Automatic','1.5L Turbo','192 hp','260 Nm','215 km/h','7.8s (0-100)','14 km/L','473 L',
      ...
      'linear-gradient(135deg,#1c1c1c 0%,#2d2d2d 50%,#1a1a1a 100%)','#c9a84c','#c8d0d8',1],
    ['C003','Mahindra','Thar',2023,'suv',150,4,'Diesel','Manual','2.2L mHawk Diesel','130 hp','320 Nm','155 km/h','12s (0-100)','11 km/L','250 L',
      ...
      'linear-gradient(135deg,#2d1b00 0%,#3d2600 50%,#1a0e00 100%)','#c9a84c','#c8a97e',1],
    ['C004','BMW','X5',2024,'suv',200,5,'Diesel','Automatic','3.0L B57 TwinTurbo','340 hp','700 Nm','250 km/h','5.5s (0-100)','12 km/L','645 L',
      ...
      'linear-gradient(135deg,#0a0f1e 0%,#1a2540 50%,#0d1a35 100%)','#4c8ac9','#8aaccc',1],
    ['C005','Tesla','Model 3',2024,'electric',180,5,'Electric','Automatic','Dual Motor AWD','358 hp','510 Nm','233 km/h','4.2s (0-100)','6.5 km/kWh','594 L',
      ...
      'linear-gradient(135deg,#0d1117 0%,#1a1f2e 50%,#0d1117 100%)','#4caa78','#7ecba3',1],
    ['C006','Ford','Mustang',2024,'sports',220,4,'Petrol','Manual','5.0L V8 Coyote','450 hp','529 Nm','270 km/h','4.6s (0-100)','8 km/L','382 L',
      ...
      'linear-gradient(135deg,#1a0a0a 0%,#2d1010 50%,#1a0505 100%)','#c94c4c','#e07070',1],
  ];

  const seedAll = db.transaction(() => seed.forEach(row => insert.run(...row)));
  seedAll();

  const newRows = db.prepare('SELECT COUNT(*) as c FROM cars').get().c;
  console.log(`✅ DB ready — ${newRows} cars in fleet`);
